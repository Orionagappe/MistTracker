...
class SPDFAdapter(DataAdapter):
    ...
    def load_data(self, config: Dict[str, Any]) -> Dict[str, np.ndarray]:
        ...
        # Solar wind has ion cyclotron waves at ~1 Hz
        fundamental = 0.5
        bx = (50 + 0.2 * np.sin(2*np.pi*fundamental*timestamps) +
              0.06 * np.sin(2*np.pi*2*fundamental*timestamps))
        by = (30 + 0.15 * np.sin(2*np.pi*fundamental*timestamps + np.pi/4) +
              0.05 * np.sin(2*np.pi*2*fundamental*timestamps + np.pi/4))
        bz = (10 + 0.1 * np.sin(2*np.pi*fundamental*timestamps - np.pi/3) +
              0.03 * np.sin(2*np.pi*2*fundamental*timestamps - np.pi/3))
        
        # Add measurement noise
        bx += np.random.normal(0, 0.05, len(timestamps))
        by += np.random.normal(0, 0.05, len(timestamps))
        bz += np.random.normal(0, 0.05, len(timestamps))
        ...
        return {
            ...
            'metadata': {
                ...
                'quality_notes': 'Real SPDF data or realistic synthetic equivalent',
            }
        }
    ...
    def load_data(self, config: Dict[str, Any]) -> Dict[str, np.ndarray]:
        ...
        return {
            ...
            'metadata': {
                ...
                'date_range': f'{config["duration_seconds"]} seconds',
                'units': 'milliseconds',
