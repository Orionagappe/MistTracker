...
$testCmd = ssh -o StrictHostKeyChecking=accept-new "$DevuanUser@$DevuanIP" "whoami" 2>&1
...
ssh -o StrictHostKeyChecking=accept-new "$DevuanUser@$DevuanIP" $suCmd 2>&1 | Out-Null
...
$psOutput = ssh -o StrictHostKeyChecking=accept-new "$DevuanUser@$DevuanIP" $psCmd 2>&1
...
# Step 4: Run hardware fitness validator (5-10 minutes) and save output
Write-Host "[4/4] Running Hardware Fitness Validator..."
Write-Host "=========================================================="
Write-Host ""

ssh -o StrictHostKeyChecking=accept-new "$DevuanUser@$DevuanIP" "node ~/misttracker/hardware-fitness-validator.js > ~/misttracker/logs/validator-results.txt 2>&1" 2>&1
