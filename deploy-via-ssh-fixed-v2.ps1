...
try {
  $result = sshpass -p $DevuanPassword ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=5 $DevuanUser@$DevuanIP "echo SSH_OK" 2>&1
...
sshpass -p $DevuanPassword ssh -o StrictHostKeyChecking=no $DevuanUser@$DevuanIP "mkdir -p $RemoteDir" 2>&1 | Out-Null
...
foreach ($file in $filesToCopy) {
  if (Test-Path ".\$file") {
    try {
      sshpass -p $DevuanPassword scp -o StrictHostKeyChecking=no ".\$file" "${DevuanUser}@${DevuanIP}:${RemoteDir}/" 2>&1 | Out-Null
...
$setupOutput = sshpass -p $DevuanPassword ssh -o StrictHostKeyChecking=no $DevuanUser@$DevuanIP $setupCommand 2>&1
...
$validationOutput = sshpass -p $DevuanPassword ssh -o StrictHostKeyChecking=no $DevuanUser@$DevuanIP "bash ~/misttracker/validation-checklist.sh" 2>&1
